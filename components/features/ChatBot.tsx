  ...
  const processInline = (line: string): JSX.Element[] => {
    ...
    while ((match = regex.exec(line)) !== null) {
      ...
      if (match[1]) {
        ...
        parts.push(
          <code key={`c-${match.index}`} className="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-sm font-mono text-primary-600 dark:text-primary-400">
            {match[4]}
          </code>
        );
      } else if (match[5]) {
    ...
    if (numMatch) {
      elements.push(
        <div key={`ol-${i}`} className="flex gap-2 ml-2 my-0.5">
          <span className="text-primary-500 font-semibold min-w-[1.2rem]">{numMatch[1]}.</span>
...
// Suggested questions
const SUGGESTIONS = [
  { text: "What candidates are in the system?", icon: "👥" },
  { text: "Suggest interview questions", icon: "🎯" },
  { text: "How does the scoring work?", icon: "📊" },
  { text: "Compare top candidates", icon: "⚡" },
];
...
export default function ChatBot() {
  ...
  const { useRealApi } = useStore();
  const { messages, isLoading, error, sendMessage, clearChat, title, chatbotStatus } = useChat();
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
              ...
              {!hasBeenOpened && (
                ...
                >
                  <div className="px-3.5 py-2 bg-gray-900/90 dark:bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-gray-700/50 dark:border-gray-200/50">
                    ...
                    <p className="text-[11px] text-gray-300 dark:text-gray-600 mt-0.5">Chat with me</p>
                  </div>
                  {/* Arrow pointing down */}
            ...
            {/* Chat bubble button - slightly larger */}
            <motion.button
              ...
              className="group"
            >
              <div className="relative w-16 h-16 text-white shadow-xl shadow-primary-500/25 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-primary-500/40">
                {/* Chat bubble shape */}
                <svg viewBox="0 0 64 64" fill="none" className="absolute inset-0 w-full h-full drop-shadow-lg">
                  <defs>
                    <linearGradient id="chatBubbleGrad" x1="0" y1="0" x2="64" y2="64" gradientUnits="userSpaceOnUse">
                      <stop stopColor="#8b5cf6" />
                      <stop offset="1" stopColor="#7c3aed" />
                    </linearGradient>
                  </defs>
                  <path d="M8 8C8 4.69 10.69 2 14 2H50C53.31 2 56 4.69 56 8V40C56 43.31 53.31 46 50 46H28L16 58V46H14C10.69 46 8 43.31 8 40V8Z" fill="url(#chatBubbleGrad)" />
                </svg>
              ...
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10">
                  ...
                  <svg viewBox="0 0 40 40" fill="none" className="absolute inset-0 w-full h-full">
                    <defs>
                      <linearGradient id="headerBubbleGrad" x1="0" y1="0" x2="40" y2="40" gradientUnits="userSpaceOnUse">
                        <stop stopColor="#8b5cf6" />
                        <stop offset="1" stopColor="#7c3aed" />
                      </linearGradient>
                    </defs>
                    <path d="M5 5C5 2.79 6.79 1 9 1H31C33.21 1 35 2.79 35 5V27C35 29.21 33.21 31 31 31H18L11 38V31H9C6.79 31 5 29.21 5 27V5Z" fill="url(#headerBubbleGrad)" />
                  </svg>
            ...
            <div className="relative flex-1 overflow-y-auto px-5 py-4 space-y-5 min-h-0">
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center py-6">
                  ...
                  >
                    <div className="w-16 h-16 rounded-3xl bg-gradient-to-br from-violet-600 via-primary-500 to-indigo-600 flex items-center justify-center shadow-xl shadow-primary-500/25">
                      <Sparkles className="h-8 w-8 text-white" />
                    </div>
                    <motion.div
                      animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0, 0.3] }}
                      transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
                      className="absolute inset-0 rounded-3xl bg-gradient-to-br from-violet-600 to-indigo-600"
                    />
                  </motion.div>
                  ...
                  <p className="text-[13px] text-gray-400 dark:text-gray-500 max-w-[260px] mb-6 leading-relaxed">
                    Ask about candidates, jobs, interviews, or recruitment best practices.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-2 w-full max-w-[330px]">
                    {SUGGESTIONS.map((suggestion, i) => (
                      <motion.button
                        ...
                        className="text-left text-xs px-3.5 py-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700/50 text-gray-600 dark:text-gray-300 hover:border-primary-400/40 hover:bg-primary-50/60 dark:hover:bg-primary-900/15 transition-all duration-200 group"
                      >
              ...
              <div className="flex items-end gap-2">
                <div className="flex-1 relative">
                  ...
                  <motion.button
                    ...
                    className="absolute right-2 bottom-1.5 w-8 h-8 rounded-xl bg-gradient-to-br from-violet-600 to-primary-600 text-white flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed transition-opacity"
                  >
                    {isLoading ? (
  ...
  return (
    ...
    >
      ...
      <div className={`flex-shrink-0 w-7 h-7 rounded-xl flex items-center justify-center shadow-sm ${
        isUser
          ? 'bg-gradient-to-br from-violet-600 to-primary-600 ring-2 ring-violet-100 dark:ring-violet-900/30'
      ...
      <div className={`max-w-[82%] ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${
          isUser
            ? 'bg-gradient-to-br from-violet-600 to-primary-600 text-white rounded-tr-md shadow-sm shadow-violet-500/10'
        ...
        {message.sources && message.sources.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {message.sources.map((source, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-lg bg-primary-50 dark:bg-primary-900/15 text-primary-600 dark:text-primary-400 border border-primary-100 dark:border-primary-800/40 font-medium"
              >
